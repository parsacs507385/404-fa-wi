...
int strCount(const char* pattern, const char* s)
{
    ...
    for (int i = 0; s[i]; i++)
    {
        ...
        {
            int isOk = 1;
            for (int j = 0, k = i; pattern[j] && s[k]; j++, k++)
            {
                if (pattern[j] == s[k])
                    continue;
                else
...
void drawInfo(int maxTempWallCount, TempWall tempWalls[maxTempWallCount])
{
    int used = 0;
    for (int k = 0; k!=maxTempWallCount; k++)
        if (tempWalls[k].i != -1) used++;
...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        BeginDrawing();
        if (showInfo)
            drawInfo(maxTempWalls, tempWalls);
        else
        {
            ClearBackground(BLACK);
            ...
            DrawText("'t' for info", GetScreenWidth() - MeasureText("'t' for info", 20) - 10, 10, 20, WHITE);
        }
        EndDrawing();
    }
