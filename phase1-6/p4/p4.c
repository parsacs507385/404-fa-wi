...
void drawMap(int rows, int cols, Entity gameboard[rows][cols], int hWalls[rows][cols], int vWalls[rows][cols], int maxTempWallCount, TempWall tempWalls[maxTempWallCount], int runnerCount, Player players[runnerCount], int activeRunner)
{
    for (int i = 0; i!=rows; i++)
    {
        for (int j = 0; j!=cols; j++)
...
int main()
{
    ...
    const int MAX_TEMP_WALLS = max(min(ROWS, COLS)/3, 1);
...
redo1:
    ...
    while (1) {
        ...
        if (isOk(ROWS, COLS, GAMEBOARD, hWalls, vWalls, MAX_TEMP_WALLS, tempWalls)) break;
    }
    // final check
    ...
    while (!WindowShouldClose())
    {
        ...
        if (isPlayerTurn && players[activeRunner].state == STATE_ALIVE)
        {
            ...
            {
                if (boundCheck(ROWS, COLS, GAMEBOARD, hWalls, vWalls, players[i].i, players[i].j, 'W', MAX_TEMP_WALLS, tempWalls))
                {
            ...
            {
                if (boundCheck(ROWS, COLS, GAMEBOARD, hWalls, vWalls, players[i].i, players[i].j, 'A', MAX_TEMP_WALLS, tempWalls))
                {
            ...
            {
                if (boundCheck(ROWS, COLS, GAMEBOARD, hWalls, vWalls, players[i].i, players[i].j, 'S', MAX_TEMP_WALLS, tempWalls))
                {
                    if (GAMEBOARD[players[i].i+1][players[i].j].type == TYPE_GOAL)
                    {
                        players[i].state = STATE_WON;
            ...
            {
                if (boundCheck(ROWS, COLS, GAMEBOARD, hWalls, vWalls, players[i].i, players[i].j, 'D', MAX_TEMP_WALLS, tempWalls))
                {
                    if (GAMEBOARD[players[i].i][players[i].j+1].type == TYPE_GOAL)
                    {
                        players[i].state = STATE_WON;
            ...
            else if (IsKeyPressed(KEY_UP))
            {
                ...
                {
                    int curs;
                    for (curs = 0; tempWalls[curs].i!=-1 && curs!=MAX_TEMP_WALLS; curs++);
                    if (curs==MAX_TEMP_WALLS)
        ...
        // logic
        int gameState = logic(ROWS, COLS, GAMEBOARD, hWalls, vWalls, &isPlayerTurn, MAX_TEMP_WALLS, tempWalls, runnerCount, players, difficulty);
        ...
        drawMap(ROWS, COLS, GAMEBOARD, hWalls, vWalls, MAX_TEMP_WALLS, tempWalls, runnerCount, players, activeRunner);
