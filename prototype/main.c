...
void updatePlayer(SDL_FRect* player, float playerX, float playerY, SDL_Texture* texture)
{
    player->x = playerX; player->y = playerY;
    // SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
    // SDL_RenderRect(renderer, player);
    SDL_RenderTexture(renderer, texture, NULL, player);
}

void updateEntity(SDL_FRect* entity, float i, float j, SDL_Texture* texture)
{
    entity->x = j; entity->y = i;
    // SDL_SetRenderDrawColor(renderer, 255, 0, 0, 255);
    // SDL_RenderRect(renderer, entity);
    SDL_RenderTexture(renderer, texture, NULL, entity);
}
...
SDL_Texture* hunterTexture = NULL;
// END TEXTURES
...
start:
    ...
    window = SDL_CreateWindow("🙏💀 TALE OF EDP 💀🙏", n*ONE_BLOCK_W, m*ONE_BLOCK_H, SDL_WINDOW_RESIZABLE);
    renderer = SDL_CreateRenderer(window, NULL);

    SDL_Surface* icon = IMG_Load("ASSETS/edp.png");
    SDL_SetWindowIcon(window, icon);
    SDL_DestroySurface(icon);

    playerTexture = IMG_LoadTexture(renderer, "ASSETS/cupcake.png");
    hunterTexture = IMG_LoadTexture(renderer, "ASSETS/edp.png");
...
goto1:
        ...
        updatePlayer(player, playerX, playerY, playerTexture);
        updateEntity(hunter, hunterY, hunterX, hunterTexture);
